...
Legacy IDs: V-96021; SV-105159

Extraction Method: Native (CLI/API JSON)
Platform: Cisco IOS-XE Router

Rule Title: The Cisco router must be configured to automatically audit account removal.
...
Auditing account removal along with an automatic notification to appropriate individuals 
will provide the necessary reconciliation that account management procedures are being 
followed. If account removal is not audited, reconciliation of account management 
procedures cannot be tracked, and unauthorized accounts may persist or authorized accounts 
may be removed without detection.

Check Text:
Review the router configuration to determine if it automatically audits account removal. 
...
STIG_ID = "CISC-ND-000120"
FINDING_ID = "V-215666"
RULE_ID = "SV-215666r960786_rule"
SEVERITY = "Medium"
CATEGORY = "STIG"
PLATFORM = "ios-xe-router"
EXTRACTION_METHOD = "native"
TITLE = "Router must automatically audit account removal"
...
def test_archive_log_config_enabled():
    """
    Test that archive log config logging is enabled to audit account removal.
    
    STIG V-215666 (CISC-ND-000120) requires that the router automatically audits 
    account removal. This is accomplished by enabling configuration archiving 
    with logging, which records all configuration changes including account removal.
    
    The test validates that:
    1. Archive logging is configured
    2. Logging is enabled
    
    This ensures all account removal actions are automatically tracked and can be 
    ...
    for device_name, device_data in devices.items():
        try:
            ...
            if not overall_compliant:
                ...
                error_parts.append("\nAccount removal is NOT being automatically audited!")
    ...
    for device, result in results.items():
        ...
        if result.get('compliant'):
            ...
            print(f"  ✓ Account removal is automatically audited")
        else:
            if 'error' in result:
